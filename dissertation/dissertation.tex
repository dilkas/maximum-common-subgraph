...
\usepackage{amsfonts, amsmath, amsthm, graphicx, subcaption, mathtools, cleveref}
...
Labelling will be formally defined in Section \ref{sec:characteristics}. For
... TODO: introduce labelling
...
\begin{algorithm}
  \SetKwData{kOriginal}{kOriginal}
  \SetKwData{kLeft}{kLeft}
  \SetKwData{pNds}{pNds}
  \SetKwData{tNds}{tNds}
  \SetKwData{successful}{successful}
  \KwData{a non-negative integer \kOriginal, a list of NDSs for each label
  ...
  \For{$i \gets 0$ \KwTo $\min\{ |\pNds|, |\tNds| \} - 1$}{
    \For{$k \gets \max \{ |\pNds[i]| - |\tNds[i]|, 0 \}$ \KwTo $\infty$}{
      \If{$k > \kLeft$}{\Return{false};}
      \If{$\bigwedge\limits_{j = k}^{|\pNds[i]| - 1} \tNds[i][j - k] + k \ge
        \pNds[i][j]$}{
        $\kLeft \gets \kLeft - k$\;
        break\;
      }
