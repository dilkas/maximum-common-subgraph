...
\begin{abstract}
  ...
  conclusions of many research papers seem overly simplistic. Moreover, we use
  our trained machine learning models to gain insight into how well the models
  can recognise instances favouring each algorithm, which features of the
  problem instances are most important, and which algorithms prefer what values
  of those features. Finally, we explore several ways to improve existing
  algorithms and create new ones: we adapt the $k{\downarrow}$ algorithm to
  handle vertex labels, and propose two ways of switching algorithms
  mid-execution.
\end{abstract}
...
instances best handled by each algorithm, perhaps training a machine learning
(ML) model to quickly choose an algorithm based on features of the input graphs
would result in a portfolio faster than any individual algorithm. This is an
...
\begin{definition}
  ...
  $\mathcal{A}$, and a performance measure $m : \mathcal{I} \times
  \mathcal{A} \to \mathbb{R}$, the \emph{per-instance algorithm selection
    problem} is to find a mapping $s : \mathcal{I} \to \mathcal{A}$ that
...
inspired by a PhD thesis on graph matching \cite{abu-aisheh_2016}):

\begin{definition} \label{def:vertex_label}
  A \emph{(vertex-)labelled graph} is a 3-tuple $G = (V, E, \mu)$, where $V$,
  $E$ are as in Definition~\ref{def:graph} and $\mu : V \to \{ 0, \dots, N
...
\begin{definition} \label{def:edge_label}
  ...
  : E \to \{ 0, \dots, M - 1 \}$ is an edge labelling function, for some $M
  \in \mathbb{N}$.
\end{definition}
...
\begin{definition}
  ...
  \zeta_2)$ are said to be \emph{isomorphic} if there are bijections $f :
  V_1 \to V_2$, $g : E_1 \to E_2$ such that
...
\begin{definition}
  Let $f : X \to Y$ be a function. Then the \emph{restriction} of $f$ to $A
  \subseteq X$ is the function $f|_A : A \to Y$ such that $\forall a \in A$,
...
\begin{example}
  ...
  one edge. Note that $G_p$ does not have an independent set of size 3 (three
  vertices with no edges between them), so the maximum number of vertices in any
  common subgraph is three, and $u_1, u_2$ must be included. The only vertex in
  $G_p$ not connected to either $u_1$ or $u_2$ is $u_3$, so any maximum common
  subgraph must be isomorphic to the subgraph induced by $\{ u_1, u_2, u_3 \}$
  in $G_p$. Indeed, there are three different solutions:
  \begin{gather*}
    \{ u_1 \mapsto v_4, u_2 \mapsto v_1, u_3 \mapsto v_3 \}, \\
    \{ u_1 \mapsto v_4, u_2 \mapsto v_2, u_3 \mapsto v_3 \}, \\
...
All labelled graphs are taken from the ARG Database \cite{foggia2001-2,
  DBLP:journals/prl/SantoFSV03}, which is a large collection of synthetic graphs
for benchmarking various graph-matching algorithms. The database contains
randomly generated graphs; 2D, 3D, and 4D meshes; and bounded valence graphs.
...
\item[meshes-CVIU11] Graphs generated from meshes modelling 3D
  objects. There are 6 pattern graphs and 503 target graphs, giving a total of
  \num{3018} instances. Both \texttt{images-CVIU11} and \texttt{meshes-CVIU11}
  datasets are described in a paper by Damiand \etal{}
...
\item[si] Bounded valence graphs, 4D meshes, and randomly generated graphs
  ...
  The \texttt{LV}, \texttt{scalefree}, and \texttt{si} datasets are described in
...
A machine learning (ML) model requires data from which to learn. We are using an R
package called \textsc{Llama} \cite{kotthoff_llama_2013, llama}, which helps to train
and evaluate ML models in order to compare algorithms. \textsc{Llama} has
previously been used to create algorithm portfolios for the travelling
salesperson problem \cite{DBLP:conf/lion/KotthoffKHT15} and the subgraph
isomorphism problem \cite{DBLP:conf/lion/KotthoffMS16}. First, we run each
algorithm on all pairs of pattern-target graphs and record the running times
(described in Section~\ref{sec:runtimes}). Then, we adapt a graph feature
extractor program used in an algorithm selection paper for the subgraph
isomorphism problem by Kotthoff, McCreesh, and Solnon
...
The algorithms were compiled with gcc 6.3.0 and run on nodes with the Ubuntu
17.04 (Zesty Zapus) operating system, dual Intel Xeon E5-2697A v4 (2.60 GHz)
processors, and 512 GB RAM. Each algorithm was set to time out after \num{1000}
s. A Makefile was created to run multiple experiments in parallel (with, e.g.,
...
We plot the ECDFs for unlabelled graphs in both databases in
...
other algorithms. Therefore, if two or more lowest runtimes are equal (as can
...
consider the clique encoding, and to mostly forget about $k{\downarrow}$.
...
better than \textsc{McSplit}, which is in turn better than
...
design $\textsc{McSplit}{\downarrow}$ is better at handling problem
...
\cite{kotthoff_llama_2013} and was successfully used in a similar study
...
graphs. We examine how well the portfolio performs compared to
individual algorithms and the VBS, how well the ML model predicts the winning of
each algorithm, how important each feature is in making a prediction, how
convinced the model is of each prediction, and which algorithms prefer what
values of various features.
...
We start by comparing the performance of all individual algorithms, the VBS, and
the \textsc{Llama} models in Figure~\ref{fig:ecdf_llama}. While there are
important differences among individual algorithms' curves (discussed in
...
\begin{table}
  ...
  \begin{tabular}{l c c}
    ...
    \bottomrule
  \end{tabular}
  \caption{The values that all four errors (approximately) converge to, for both
    types of labelling.}
  \label{table:errors}
\end{table}
...
Recall that the clique approach reduces an MCS instance to an association graph.
...
$\mu_{\mathcal{G}} : V_{\mathcal{G}} \to \{ 0, \dots, N - 1 \}$ and
$\mu_{\mathcal{H}} : V_{\mathcal{H}} \to \{ 0, \dots, N - 1\}$. We
...
\begin{remark}
  ...
  $A_{\mathcal{G}}, A_{\mathcal{H}}$, where $A_{\mathcal{G}} :
...
\begin{proposition}
  ...
  this defines a function $\mu_{\mathcal{G'}} : V_{\mathcal{G'}} \to \{ 0,
...
To sum up, we showed how the decision about switching algorithms mid-execution
could be made based on the runtime data of the two algorithms on a wide range of
instances, while varying the initial size of the incumbent. It remains an open
question as to whether this approach can be implemented in a practical way.
The runtime data might suggest that switching algorithms is never the right
call. Otherwise, the choice will depend on a number of features, and it remains
to be seen whether those features can be tracked efficiently, and whether the
extra costs of considering the switch, and actually re-encoding the problem
instance are worth the predicted gains in performance. The list of possible
features can even be expanded to consider not just the two graphs of the
unsolved MCS instance, but the corresponding association graph as well.
