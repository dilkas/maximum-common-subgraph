...
namespace
{
  ...
  auto read_word_vf(std::ifstream & infile) -> unsigned
  {
    unsigned char a, b;
  ...
  auto read_vf(const std::string & filename, bool unlabelled, bool no_edge_labels, bool undirected) -> VFGraph
  {
    ...
    result.size = read_word_vf(infile);
    if (! infile)
    ...
    for (unsigned r = 0 ; r < result.size ; ++r) {
      unsigned l = read_word_vf(infile) >> (16 - k1);
      if (unlabelled)
        l = 0;
    ...
    for (unsigned r = 0 ; r < result.size ; ++r) {
      int c_end = read_word_vf(infile);
      if (! infile)
        throw GraphFileError{ filename, "error reading edges count" };

      for (int c = 0 ; c < c_end ; ++c) {
        unsigned e = read_word_vf(infile);
        ...
        if (unlabelled) {
          ...
          unsigned l = (read_word_vf(infile) >> (16 - k1)) + 1;
...
auto read_word_lad(std::ifstream & infile) -> unsigned
{
  unsigned x;
  infile >> x;
  return x;
...
auto read_lad(const std::string & filename) -> VFGraph
{
  ...
  result.size = read_word_lad(infile);
  if (! infile)
    throw GraphFileError{ filename, "error reading size" };
  ...
  for (unsigned r = 0 ; r < result.size ; ++r) {
    unsigned c_end = read_word_lad(infile);
    if (! infile)
      throw GraphFileError{ filename, "error reading edges count" };

    for (unsigned c = 0 ; c < c_end ; ++c) {
      unsigned e = read_word_lad(infile);
...
auto main(int argc, char * argv[]) -> int
{
  try {
    ...
    display_options.add_options()
      ...
      ("lad",                                   "Use LAD format instead of VF")
      ;
