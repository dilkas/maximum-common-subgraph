...
static struct argp_option options[] = {
  ...
  {"depth", 'e', "depth", 0, "Depth at which to switch to clique (0 is the root node)"},
  {"labelling", 'n', "labelling", 0, "The labelling percentage (33 by default)"},
  { 0 }
};
...
static error_t parse_opt (int key, char *arg, struct argp_state *state) {
  ...
  case 't':
    ...
    break;
  case 'e':
    arguments.depth = std::stoi(arg);
    break;
  case 'n':
    arguments.labelling = std::stoi(arg);
    break;
  case ARGP_KEY_ARG:
...
void run_clique(const Graph & g0, const Graph & g1, vector<VtxPair> & incumbent,
                vector<VtxPair> & current, vector<Bidomain> & domains,
                vector<int> & left, vector<int> & right) {
  // a hopefully slightly faster version of modular_product from the clique
  // algorithm
  ...
  // construct vertices
  for (auto bidomain : domains)
    for (int i = 0; i < bidomain.left_len; i++)
      for (int j = 0; j < bidomain.right_len; j++)
        association.insert({{left[bidomain.l + i], right[bidomain.r + j]},
              next_vertex++});

  // add edges
  for (auto & a1 : association)
    for (auto & a2 : association)
      if (a1.first.first != a2.first.first &&
          a1.first.second != a2.first.second &&
          g0.adjmat.at(a1.first.first).at(a1.first.first) ==
          g1.adjmat.at(a1.first.second).at(a1.first.second) &&
          g0.adjmat.at(a2.first.first).at(a2.first.first) ==
          g1.adjmat.at(a2.first.second).at(a2.first.second) &&
          g0.adjmat.at(a1.first.first).at(a2.first.first) ==
          g1.adjmat.at(a1.first.second).at(a2.first.second) &&
          g0.adjmat.at(a2.first.first).at(a1.first.first) ==
  ...
  Params params;
  params.abort = &abort_due_to_timeout;
  params.start_time = std::chrono::steady_clock::now();
  params.connected = arguments.connected;
  params.prime = incumbent.size() - current.size();

  Result result = mcsplit_run({association, edges}, params, first_graph);
  if (current.size() + result.isomorphism.size() > incumbent.size()) {
    incumbent = current;
    for (auto & pair : result.isomorphism)
      incumbent.push_back(VtxPair(pair.first, pair.second));
  }

  /*
  // Some tests for debugging
  for (auto & p : result.isomorphism) {
    if (g0.label.at(p.first) != g1.label.at(p.second)) {
      std::cerr << "Oops1!" << std::endl;
    }
  }
  for (auto & p : result.isomorphism) {
    for (auto & q : result.isomorphism) {
      if (g0.adjmat.at(p.first).at(q.first) != g1.adjmat.at(p.second).at(q.second)) {
...
void solve(const Graph & g0, const Graph & g1, vector<VtxPair> & incumbent,
           vector<VtxPair> & current, vector<Bidomain> & domains,
           vector<int> & left, vector<int> & right,
           unsigned int matching_size_goal, int depth = 0)
{
  ...
  if (arguments.depth != -1 && depth >= arguments.depth) {
    run_clique(g0, g1, incumbent, current, domains, left, right);
    return;
  }
