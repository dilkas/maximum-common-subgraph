...
different datasets. We plot most of the data, except for two statistics, which % TODO: here!
are ought to be mentioned as numbers. 99.81\% of the labelled graphs are connected, compared to
93.19\% of the unlabelled dataset.

In the ARG database, all graphs are assigned into pairs (A and B) with no
...
\ref{fig:mcs_features1} and \ref{fig:mcs_features2}. 0.98\% of the graphs have a
single loop, other graphs have no loops.

For unlabelled graphs, we will only consider graphs that are part of a pair of graphs solved by at least one algorithm. Since most of the data is heavily skewed, Figures \ref{fig:sip_features1} and \ref{fig:sip_features2} show the distributions as density plots.
