...
plots \cite{10.2307/2334448}: for each unit of time on the horizontal axis, the value on
the vertical axis represents what part of the problem instances was solved in that
...
\begin{figure} % TODO: update this with the real thing
  \centering
...
model managed to solve 45302, compared to 45223 solved by % TODO: update the numbers
$\textsc{McSplit}\downarrow$. In other words, it was able to close 32\% of the
gap between our lower and upper bounds. Figure \ref{fig:ecdf_unlabelled_llama}
shows how the ML model compares to the VBS and the single best solver
$\textsc{McSplit}\downarrow$ (note that the vertical axis starts at 0.8 rather than
0). Unsurprisingly, the model outperforms $\textsc{McSplit}\downarrow$, but does
not reach the best possible performance represented by the VBS.
