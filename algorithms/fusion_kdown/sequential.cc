...
namespace
{
  ...
  struct SIP
  {
    ...
    auto solve(
               Domains & domains,
               Assignments & assignments, unsigned depth = 1) -> bool
    {
      if (*params.abort)
        return false;

      if (params.depth != 0 && depth >= params.depth)
        return run_clique(domains, assignments);
      ...
      for (auto & branch_value : branch_values) {
        ...
        if (unit_propagate(new_domains, assignments))
          if (solve(new_domains, assignments, depth + 1))
            return true;
    ...
    auto run_clique(Domains & domains, Assignments & assignments) {
      unsigned next_vertex = 0;
      Association association;
      AssociatedEdges edges;

      for (auto & domain : domains) {
        for (auto vertex = domain.values.find_first(); vertex != Bitset_::npos ;
             vertex = domain.values.find_next(vertex)) {
          association.insert({{domain.v, vertex}, next_vertex++});
        }
      }

      for (auto constraint : adjacency_constraints) {
        for (auto & domain : domains) ...
      }
